# Verify — Post-Edit Workspace-Block Recovery

Cold-path content extracted from `verify/SKILL.md` Phase 7. Documents the recovery flow when `check-workspace-scope.py` PreToolUse hook blocks an `Edit`/`Write` on a `.ivy` file outside the active workspace during a Phase 7 fix application.
