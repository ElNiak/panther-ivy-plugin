# Verify workflow — glossary

Six terms `workflow-verify` uses without further definition. Each is a calibrated word with a fixed meaning in the plugin; do not paraphrase.
...
| Iron law | A binding constraint with a deterministic enforcement site (a hook, a precondition, or a gate-citation requirement). The four iron laws are documented canonically in `.claude/rules/iron-laws.md`; each rigid workflow inlines a 1-2 sentence summary plus a pointer to the canonical wording. |
...
| `pending_dispatch` | Journal event (`workflow_state.append_pending_dispatch`) used to hand off control to another workflow across a turn boundary. The emitter writes the event then clears its `active-workflow` flag; navigate Phase 1 Step 2c consumes the event on the next turn. The journal record is load-bearing for `/nct-observability` and the agent-dispatch / mcp-tool-reliability recovery contracts. |
