---
name: nct-health
description: "Run the full 9-step diagnostic sequence for Ivy LSP + MCP integration — validates functional correctness, not just liveness"
arguments: []
---

> **Shortcut command** — runs the full 9-step diagnostic sequence. For quick liveness checks and restart, use the `triage` workflow.
...
See the `ivy-toolkit` knowledge skill for MCP architecture and LSP invocation patterns.
