...
```
Call: mcp__ivy_tools__ivy_workspace(action="set", target="minip")
```

If the index is stale or missing, rebuild it:

```
Call: mcp__ivy_tools__ivy_index(protocol="minip")
```
...
1. **Type structure:** `mcp__ivy_tools__ivy_find_variants(type_name="<type>")` — validates the type exists and shows current structure
2. **Ser/deser mapping:** `mcp__ivy_tools__ivy_serdes_correlation(type_name="<type>")` — finds which ser/deser files handle this type
3. **Impact categorization:** `mcp__ivy_tools__ivy_change_impact(type_name="<type>", change_type="<add_field|add_variant>")` — returns `auto_propagate`, `manual_review`, and `unaffected` file lists
...
1. **Read** the file via `mcp__serena__read_file`
...
5. **On approval**, apply via `mcp__serena__replace_content`
...
1. Compile the server-side test: `mcp__ivy_tools__ivy_compile(relative_path="minip_tests/server_tests/ping_server_test.ivy")`
2. If that passes, compile the client-side test: `mcp__ivy_tools__ivy_compile(relative_path="minip_tests/client_tests/ping_client_test.ivy")`
...
1. Write back the full original content via `mcp__serena__replace_content`
