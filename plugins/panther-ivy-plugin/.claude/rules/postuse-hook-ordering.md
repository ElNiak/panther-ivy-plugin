...
| `posttooluse/gates/run-gate.py --id g2\|g3\|g5` (logic in `posttooluse/gates/gate_handlers.py`) | per-gate inputs: G2/G3 read `tool_input.file_path` + `WorkflowContext.current()` + `scaffold-state.yaml`; G5 reads `tool_result` artifacts + `find_protocol_dir()` + active-workflow state + `scaffold-state.yaml` | `gate_dispatched` journal event; per-gate dispatch directive via `additional_context` |
