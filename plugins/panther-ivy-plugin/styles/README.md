...
|-------------------|----------------------------------------------|------|
| `overlays/`       | `hooks/scripts/prompt/style.py` (UserPromptSubmit) | Per-workflow style overlays layered on top of the user's selected output style. One file per workflow (build, verify, review, triage, navigate). |
| `summaries/`      | `hooks/scripts/render/summary/main.py` (Stop) | Per-workflow summary templates rendered at session end from the workflow journal. One file per workflow. |
