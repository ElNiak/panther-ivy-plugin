...
The mandatory checklist Claude follows before proposing a fix to a verification or compilation failure. The host skill (`knowledge-verification-failures`) points here whenever `ivy_verify` / `ivy_check` fails and a fix is being prepared.
