...
After each `Write`/`Edit` on a `.ivy` file, the builder agent dispatches critics inline (per `scaffold-ops/SKILL.md` Phase 3 G2/G3 hard-gate); the PostToolUse hooks `posttooluse/gates/run-gate.py --id g2` and `posttooluse/gates/run-gate.py --id g3` are a backstop. Critic prompts come from the verbatim G2/G3 catalog templates kept under `skills/ivy/references/critic_prompts/`:
