...
1. **Attempt-counter gate** (before each compile attempt):
   ...
   - If `count >= 5`, ESCALATE via `AskUserQuestion`:
     ...
     - **Switch workflow** — emit `append_pending_dispatch(target_workflow="workflow-verify", reason="Compile loop capped on <layer>")` (or `build` if structural rethink is needed), clear the active-workflow flag.
...
After all layers are written and compile, update phase to `"written"` via `ivy_workflow_state(action="set", workflow="workflow-build", phase="written", protocol="<protocol>")`.
