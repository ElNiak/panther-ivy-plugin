---
name: workflow-build
description: "Deprecated under approach E orchestrator refactor (2026-04-28). Functionality moved to skills/build-ops. Will be deleted in Phase F."
...
- `NO_LAYER_WITHOUT_SCAFFOLD` — No Write/Edit on layer N until `ivy_diagnostics(mode=structural)` is SOUND on the predecessor layer. Compile success is necessary but not sufficient.
- `STALENESS_RULE` — Re-run any tool result whose include closure has been edited since the result timestamp. Tool output from a prior turn is NOT acceptable evidence if any source file matched by the include closure changed.
