...
- Canonical Ivy 1.7 syntax reference (owned by ivy-syntax skill; load via `Skill(skill="panther-ivy-plugin:ivy-syntax")`). The pointer rule `.claude/rules/ivy-patterns.md` is reference-only (loaded by name, not auto-injected on `.ivy` edits).
- `.claude/rules/iron-laws.md`
- NCT/NACT/NSCT methodology reference (owned by methodology skill; load via `Skill(skill="panther-ivy-plugin:methodology")`). The pointer rule `.claude/rules/nct-methodology.md` is reference-only (loaded by name, not auto-injected on `.ivy`/`.spec` edits).
