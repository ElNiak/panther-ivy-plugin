...
Cold-path content extracted from `build/SKILL.md` Phase 1 Step 2. Loaded by `auto-load-skill-references.py` when `build` is invoked, or read explicitly when authoring a new protocol model and needing to choose a model-architecture stance up front.
