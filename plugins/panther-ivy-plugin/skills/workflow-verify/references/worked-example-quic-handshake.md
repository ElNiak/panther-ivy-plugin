...
A full end-to-end walkthrough of one verify cycle: compile, fail, diagnose,
...
The G4 verification gate (PostToolUse) immediately dispatches three context-isolated critics from `cross-cutting-reflection-patterns`. They cite verifier-pattern catalog `#410` (missing guard) — the trace shows `frame.crypto.handle` firing while `conn_seen(scid)` is still `false`, meaning a precondition is missing on the `before` block. Verdict: `UNSOUND(#410, missing-guard, quic_server_test_handshake.ivy:142)`.
...
The unified diff is applied via the `Edit` tool. The PostToolUse `assess-modeling.py` hook does NOT fire G2 on Phase 7 verify-time edits (G2 is build-time only — see `cross-cutting-reflection-patterns/references/gates.md` §G2/G3 workflow scope).
...
`Skill(skill="panther-ivy-plugin:cross-cutting-completion-gate")` runs the 5-step IDENTIFY → RUN → READ → VERIFY → THEN-claim sequence. With the fresh OK + SOUND verdict + the journal showing the fix and re-verify in the same turn, the gate passes. The user-facing "verification passed" claim is now licensed.
...
| `<dispatch-context>` capability contract | spec-analyst dispatch in Phase 6. |
| Counterexample interpretation pattern `#410` | G4 critic verdict + spec-analyst diagnosis. |
...
Refresh trigger: when a new verifier-pattern is added to `verifier_patterns.md`, re-derive the JSON / trace format here so the example stays representative of current verifier output.
