...
5. **Iteration caps:** Check the deserializer for hardcoded iteration limits (e.g., `current_ping_size == 5`). Adding a new variant may require updating these caps. See `references/minip-examples.md` for the MiniP Add-Variant worked example (Ivy definition, tag dispatch in ser/deser, state transitions).
...
- **Hardcoded constants (MiniP-specific).** See `references/minip-examples.md` for the `payload_length = 12` and `current_ping_size == 5` semantic constants that `ivyc` cannot validate.
...
- **`ivy-syntax`** — load this skill via the `Skill` tool and consult its `references/serializer-patterns.md` for the C++ state-machine internals.
