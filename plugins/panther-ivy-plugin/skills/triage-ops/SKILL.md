...
- `references/full-health-check.md` — 9-step deep-validation runbook invoked when `args="full-health-check"`. Includes per-phase `ivy-refiner-agent` reviewer dispatches and content-validation procedures (CLI-tool PATH parity, layer-staging consistency, cross-layer goToDefinition).
