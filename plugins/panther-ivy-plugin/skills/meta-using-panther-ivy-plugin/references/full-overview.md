# panther-ivy-plugin — full overview

This file holds the full overview content of `meta-using-panther-ivy-plugin`. The slim SKILL.md body (loaded at SessionStart and at every Skill-tool invocation of this skill) covers only the load-bearing primer: the 1% rule, methodology routing, the iron-law table, and the workspace pointer.

Read this file when you need the full skill catalog, the integration details, the workspace-mode taxonomy, or the deferred fifth iron law — none of which the slim SKILL.md restates.
