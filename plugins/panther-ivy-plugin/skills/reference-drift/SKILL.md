...
| **Hook-emitted** | A hook script prints a string that names a non-existent target; the user/Claude is told to invoke something that does not exist | CRITICAL | Historical case (now resolved): `render/summary/main.py` once printed `Skill(skill="panther-ivy-plugin:cross-cutting-knowledge-capture")` for a skill that did not exist; the offending emit-site has since been removed. |
...
| **Code-level orphan** | A Python `elif`, `case`, or dispatch table branch names a target that has been renamed; the branch is dead code | INFO | `hooks/scripts/render/summary/main.py:101` has `elif workflow == "navigate":` (orphan post-rename) |
