...
grep -rnE '\b(spec-analyst|model-reviewer|traceability-agent|...)\b' .claude/rules/
...
|---|---|---|---|
| **Runtime-fatal** | The named target is invoked at runtime (`Skill(...)`, `Agent(...)`); call returns "Unknown skill / agent not found" | CRITICAL | `meta-self-mod-ops/SKILL.md:92` calls `panther-ivy-plugin:model-reviewer` (no such agent) |
