...
**Do not guess.** A wrong confident verdict is worse than an honest `UNSURE`. The measure that matters is conditional accuracy — when you say `SOUND`, are you right? A false `SOUND` here is the exact failure mode this gate exists to prevent. If your reasoning hits a wall, return `UNSURE` with a short reason.
