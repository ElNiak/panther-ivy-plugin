...
**G6 knowledge-capture detection (cold-start-eligible turns only).** If neither warm-resume branch fired (no fresh `pending_dispatch`, no PROJECT.md non-idle mode), check whether the previous session's `session_end` is unpaired:

1. Read journal `last_n=200` (wider than the standard 20-entry window so multi-session pairing works).
2. Find the most recent `session_end` at ts=T_s. If none, fall through to cold start.
3. Find the most recent `gate_verdict{gate=g6}` at ts=T_v6. If T_v6 > T_s, the session is already paired — fall through.
4. If env var `IVY_DISPATCH_G6 == "0"`, emit `[ivy-noop] G6 skipped (env opt-out)` and fall through.
5. Build candidate batch via `lib.workflow_state.knowledge_capture.extract_candidates(journal, since_ts=<prior session_start>, until_ts=T_s)`.
6. Pre-filter via `lib.workflow_state.knowledge_capture.apply_dedup(candidates, journal)`.
7. If batch empty, emit `[ivy-noop] G6: no fresh candidates` and fall through.
8. Append `gate_dispatched{gate=g6, candidate_count=N, session_end_ts=T_s}`.
9. Dispatch `g-knowledge-critic` ×3 in parallel per `skills/ivy/references/parallel-dispatch.md` (~90s blocking).
10. Aggregate batch verdict; append `gate_verdict{gate=g6, verdict=..., candidate_count=N}`.
11. Per-candidate aggregation via `aggregate_per_candidate_votes`:
    - ≥2 KEEP → append `knowledge_captured(candidate_id, summary, evidence_paths, confidence="high")`.
    - ≥2 DROP → no journal write.
    - DEFER (≥2 DEFER, or 1-1-1 split) → AskUserQuestion(KEEP / DROP / SKIP). KEEP writes `confidence="user-confirmed"`; DROP and SKIP do not write; SKIP additionally does NOT add candidate_id to the dedup set (eligible for re-vote next session).
12. Branch consumes the turn (do not also run cold start).

**F8 ordering rationale:** workflow-resume turns prioritize the resumed workflow over knowledge capture. Both pending_dispatch warm-resume AND PROJECT.md warm-resume count as workflow-resumes; G6 sits after both so neither resume pays the ~90s G6 cost. **G6 dispatch is gated indirectly by `is_session_active()`** (see `record/session-end.py:25`) — sessions without Ivy activity skip `session_end` entirely, so G6 never fires for them. By design (F6).
