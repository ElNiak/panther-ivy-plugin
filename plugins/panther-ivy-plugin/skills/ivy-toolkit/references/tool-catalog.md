...
| `gate_dispatched` | gate-trigger hooks (`posttooluse/gates/run-gate.py --id g2\|g3\|g5` via `posttooluse/gates/gate_handlers.py`, `record/workflow-error.py` for G4) | `gate`, `trigger`, `artifact`, `layer`, `methodology` |
...
Detailed payload shapes for `gate_verdict` (calibrated `SOUND` / `UNSOUND` / `ABSTAIN` semantics, required and optional fields) live in `.claude/rules/gate-verdicts.md`, which auto-loads on every skill activation. `plan_approved` and `workflow_resumed` are consumed by the orchestrator's Phase 1.5 in `skills/ivy/SKILL.md` (the orchestrator absorbed the navigate role; the legacy `workflow-navigate` skill no longer exists).
