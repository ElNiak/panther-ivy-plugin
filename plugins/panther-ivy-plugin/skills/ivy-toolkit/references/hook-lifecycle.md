...
(The pre-Phase-E `prompt/style.py` and `route-user-prompt.py` hooks were removed; intent classification now lives in the orchestrator skill `skills/ivy/SKILL.md`, and style composition is driven by `.claude/rules/output-style.md` and the per-skill body, not a separate hook.)

## SessionStart Hooks

Fire once when the Claude Code session starts. No matcher. Ordering matters: `workspace/detect.py` (step 2) exports `IVY_WORKSPACE_ROOT`, `IVY_MCP_LOG_PATH`, `IVY_LSP_LOG_PATH`, and related env vars; later hooks (`cleanup/stale-pids.py`, `mcp/indexing-wait.py`) consume them. Reordering these breaks env-var propagation and causes the consumers to fall back to `/tmp` defaults.

1. `journaling/contract-check.py` — verifies `.claude/rules/journaling-contract.md` is present and parseable; fails the load if absent
2. `workspace/detect.py` — auto-detects `.ivyworkspace` markers via in-process `ivy_lsp.core.workspace.context.WorkspaceContext.detect` (with pure-Python fallback), writes `IVY_WORKSPACE_ROOT`, `IVY_LSP_LOG_PATH`, `IVY_MCP_LOG_PATH`, `IVY_SESSION_ID`, `IVY_MCP_PID_FILE`, and (conditionally) `IVY_ACTIVE_WORKSPACE` to `CLAUDE_ENV_FILE`
...
5. `prompt/using-plugin.py` — emits the orchestrator priming (1% rule, methodology routing, iron-laws summary, workspace contract)
6. `mcp/indexing-wait.py` — waits up to 30 s for the MCP `[MCP-READY]` sentinel; SIGTERM-handled with a one-shot envelope guarantee
...
|------|-----------|--------|
| pretooluse/block-direct-ivy.py | `Bash` commands containing `ivy_check`, `ivyc`, `ivy_show`, or `ivy_to_cpp` | Advisory hint; emits `[ivy-block]` system message + MCP-tool suggestion table; **always exits 0** (does not block) |
| workspace/scope.py | `Write` or `Edit` on `.ivy` files | Blocks writes to files outside the active workspace; reads across workspaces are always allowed |
| mcp/health.py | Any `mcp__.*ivy` tool | Two-tier liveness check (PID + TCP sidecar); blocks after 3 consecutive failures |
| mcp/indexing-ready.py | Any `mcp__.*ivy` tool | Four readiness signals (LSP indexed, offline index, MCP prepopulated, MCP-READY); blocks via `permissionDecision: "deny"` until indexing completes; degrades to advisory after 6 denials |
...
`render/tool-result.py` reads the active workflow overlay (set by `prompt/style.py` on each prompt) to choose verbosity, structure, and tone. The same verification failure renders differently in the `verify` workflow (diagnostic prose) versus the `review` workflow (summary table).
...
Fire when Claude Code receives a notification event (e.g., MCP server disconnect alert). No matcher. Ordering: `mcp/disconnect-notify.py` fires first so the user sees the alert promptly; `observe.py` records it last so the log captures the full notification.

| Hook | Applies to | Effect |
|------|-----------|--------|
| mcp/disconnect-notify.py | All notifications (no matcher) | Detects MCP-disconnect notifications and surfaces them to the user promptly |
