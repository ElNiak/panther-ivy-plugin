...
After each `Write`/`Edit` on a `.ivy` file, the builder agent dispatches critics inline (per `build-ops/SKILL.md` Phase 3 G2/G3 hard-gate); the PostToolUse hooks `assess-modeling.py` and `assess-testspec.py` are a backstop. Critic prompts come from the verbatim G2/G3 catalog templates kept under `references/critic_prompts/`:
