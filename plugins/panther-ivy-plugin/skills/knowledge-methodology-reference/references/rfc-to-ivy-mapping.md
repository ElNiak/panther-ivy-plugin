...
Concrete patterns for translating RFC normative language into Ivy assertions. The host skill (`knowledge-methodology-reference`) points here when working through requirement extraction or spec authoring; defer to `knowledge-ivy-writing-guide` for raw language syntax.
