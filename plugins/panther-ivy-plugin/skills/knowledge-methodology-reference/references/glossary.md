...
Eight Ivy / methodology terms used across `knowledge-methodology-reference` without further definition.

| Term | Definition |
|---|---|
| NCT | Normal Compliance Testing. The default methodology: test an IUT against RFC normative requirements (`MUST` / `SHOULD` / `MAY`). Workflow path: `build` → `verify` → `review`. Reference skills loaded: `knowledge-specification-patterns`, `knowledge-ivy-writing-guide`, `knowledge-verification-failures`. |
| NACT | Normal Adversarial Compliance Testing. Security-focused: model an attacker, prove the IUT either resists the attack or detect the violation. Workflow path: `build` (with `knowledge-apt-attack-patterns` scope) → `verify`. Adds the APT 6-stage attack lifecycle and around-block monitors. |
