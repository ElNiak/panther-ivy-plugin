...
Full reference for NCT, NACT, and NSCT methodologies: theory, the 14-layer formal-model template, the 10-step NCT workflow, NACT attack composition, NSCT simulation, APT 6-stage table, directory structure, code examples, and red-flag tables. Ships inside the plugin so auto-loading on `.ivy`/`.spec` edits resolves locally. For overview and decision guidance, see the parent `SKILL.md`.
...
The core 14-layer template is always present. These additional layers sit alongside the canonical 14 and are protocol-specific:
...
Use these protocol properties to choose which layers and optional modules to include when scaffolding a new protocol model:
...
## NCT Directory Structure (canonical)
...
## Common NCT Mistakes (quick-reference)
...
## Common NACT Mistakes (quick-reference)
...
## Common NSCT Mistakes (quick-reference)
