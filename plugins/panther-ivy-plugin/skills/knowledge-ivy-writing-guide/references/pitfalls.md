...
Cross-cutting Ivy-language pitfalls Claude should avoid when authoring or editing protocol specs. The host skill (`knowledge-ivy-writing-guide`) points here when verifying *language-level* decisions; defer to `ivy-error-patterns` for the full error-to-fix lookup table with code examples.
